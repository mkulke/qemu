...
static int set_generic_regs(int cpu_fd, hv_register_assoc *assocs,
                            size_t n_regs)
{
    struct mshv_vp_registers input = {
        .count = n_regs,
...
static int set_fpu_regs(int cpu_fd, const struct FloatingPointUnit *regs)
{
    struct hv_register_assoc *assocs;
...
static int set_xc_reg(int cpu_fd, uint64_t xcr0)
{
    ...
    ret = set_generic_regs(cpu_fd, &assoc, 1);
    if (ret < 0) {
...
static int set_msrs(int cpu_fd, GList *msrs)
{
    ...
    ret = set_generic_regs(cpu_fd, assocs, n_msrs);
    g_free(assocs);
    if (ret < 0) {
