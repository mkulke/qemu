...
typedef struct MshvMemoryManager {
    size_t n_slots;
    GList *slots;
    QemuMutex mutex;
} MshvMemoryManager;

static MshvMemoryManager *mem_manager;
...
static void publish_slots(GList *new_head, GList *old_head,
                          MshvMemorySlot *removed_slot)
{
    qatomic_store_release(&mem_manager->slots, new_head);
...
/* Needs to be called with mem_manager->mutex held */
static int remove_slot(MshvMemorySlot *slot)
{
    GList *old_head, *new_head;

    old_head = qatomic_load_acquire(&mem_manager->slots);
                                   ...
                                   uint64_t size, bool readonly)
{
    ...
    if (mem_manager->n_slots >= MSHV_MAX_MEM_SLOTS) {
 ...
 *
 * Needs to be called with mem_manager->mutex or RCU read lock held.
 */
                                               ...
                                               uint64_t userspace_addr)
{
    ...
    found = g_list_find_custom(mem_manager->slots, &ref_slot,
...
MshvRemapResult mshv_remap_overlap_region(int vm_fd, uint64_t gpa)
{
    ...
    WITH_RCU_READ_LOCK_GUARD() {
        head = qatomic_load_acquire(&mem_manager->slots);
     ...
     */
    assert(mem_manager);
    QEMU_LOCK_GUARD(&mem_manager->mutex);

    /* return early if no slot is found */
    gpa_slot = find_mem_slot_by_gpa(mem_manager->slots, gpa);
    if (gpa_slot == NULL) {
        return MshvRemapNoMapping;
    }

    /* return early if no overlapping slot is found */
    overlap_slot = find_overlap_mem_slot(mem_manager->slots, gpa_slot);
    if (overlap_slot == NULL) {
        return MshvRemapNoOverlap;
    }
                       ...
                       uint64_t userspace_addr)
{
    ...
    overlap_slot = find_overlap_mem_slot(mem_manager->slots, slot);
    if (overlap_slot) {
...
void mshv_init_memory_slots(void)
{
    mem_manager = g_new0(MshvMemoryManager, 1);
