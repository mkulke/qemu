...
static void test_find_mem_entry_by_userspace_addr(void)
{
    GList *entries = NULL;
    ...
    found = mshv_find_mem_entry_by_userspace_addr(entries, base1 + 0x8000);
    g_assert(found == entry1);

    /* Test address within entry2 */
    found = mshv_find_mem_entry_by_userspace_addr(entries, base2 + 0x100000);
    g_assert(found == entry2);

    /* Test address outside any range */
    found = mshv_find_mem_entry_by_userspace_addr(entries, 0xdeadbeef);
    g_assert(found == NULL);
}
...
int main(int argc, char **argv)
{
    ...
    g_test_add_func("/accel/mshv/mem-util/find_mem_entry_by_userspace_addr",
                    test_find_mem_entry_by_userspace_addr);
